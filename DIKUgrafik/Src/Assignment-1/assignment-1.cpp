...
std::vector<glm::vec3> GenerateLinePixels(int x1, int y1, int x2, int y2)
{
    ...
    // implement the LineRasterizer - if you do not implement it the program will crash!
    
	int dx = x2 - x1; //Calculate change in x from start to finish
	int abs_2dx = std::abs(dx) << 1; //Absolute and times 2
	int x_step = (dx < 0) ? -1 : 1; //sign of dx

	int dy = y2 - y1; //Calculate change in y from start to finish
	int abs_2dy = std::abs(dy) << 1; //Absolute and times 2
	int y_step = (dy < 0) ? -1 : 1; //sign of dy;

	bool x_dominant = (abs_2dx > abs_2dy); // Shallow slope, x dominates

	int x = x1;
	int y = y1;

	if (x_dominant) {
		bool left_right = (x_step > 0); //Which direction are we moving?
		int d = abs_2dy - (abs_2dx >> 1); //Initialize the decision variable
		for (;;) {
			pixels.push_back(glm::vec3(float(x), float(y), 0.5f)); //extends vector with new point
			if ( x == x2 ) return pixels; //if done, return
			if ((d > 0 ) || (( d == 0 ) && left_right)) {
				y += y_step;	//Update y if in the NE case
				d -= abs_2dx;	//Update decision variable if in the NE case using dp+1 = dp + 2(dy - dx)
			}
			x += x_step; //These two lines are always performed, no matter if we're in E or NE case
			d += abs_2dy;
		}
	}
	else {
		...
		for (;;){
			pixels.push_back(glm::vec3(float(x), float(y), 0.5f));
			if (y == y2 ) return pixels;
			if ((d > 0) || ((d == 0) && left_right)) {
				x += x_step;
				d -= abs_2dy;
			}
    ...
    // compute the pixel coordinates and save them in the std::vector<glm::vec3> pixels
